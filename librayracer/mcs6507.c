...
static void execute_decoded_operation(racer_mcs6507 *cpu) {
	...
	switch (cpu->operation.code) {
		...
		case 0x61: case 0x65: case 0x69: case 0x6d: case 0x71: case 0x75: case 0x7d: case 0x79: {
			...
			set_status(cpu, MCS6507_STATUS_CARRY, overflow);
			break;
		}
		...
		case 0x4a:
			set_status(cpu, MCS6507_STATUS_CARRY, cpu->accumulator & 0x1);
			cpu->accumulator >>= 1;
			break;
		...
		case 0x2a:
			set_status(cpu, MCS6507_STATUS_CARRY, cpu->accumulator & 0x80);
			cpu->accumulator <<= 1;
			cpu->accumulator &= 0xff;
			break;
